	...
	IDirectDraw7* pdd, LPCSTR szBitmap, int dx, int dy)
{
	...
	//
	HBITMAP hbm = (HBITMAP)LoadImageA(GetModuleHandle(NULL), szBitmap,
		IMAGE_BITMAP, dx, dy, LR_CREATEDIBSECTION);
	if (hbm == NULL) {
		hbm = (HBITMAP)LoadImageA(NULL, szBitmap, IMAGE_BITMAP, dx, dy,
	...
	ddsd.dwWidth = static_cast<DWORD>(bm.bmWidth);
	ddsd.dwHeight = static_cast<DWORD>(bm.bmHeight);
...
HRESULT DDReLoadBitmap(IDirectDrawSurface7* pdds, LPCSTR szBitmap)
{
	...
	if (hbm == NULL) {
		hbm = (HBITMAP)LoadImageA(NULL, szBitmap, IMAGE_BITMAP, 0, 0,
			LR_LOADFROMFILE | LR_CREATEDIBSECTION);
	}
	if (hbm == NULL) {
	...
	HRESULT hr = DDCopyBitmap(pdds, hbm, 0, 0, 0, 0);
	if (hr != DD_OK) {
	...
	IDirectDrawSurface7* pdds, HBITMAP hbm, int x, int y, int dx, int dy)
{
	...
	if ((hr = pdds->GetDC(&hdc)) == DD_OK) {
		StretchBlt(hdc, 0, 0, static_cast<int>(ddsd.dwWidth),
			static_cast<int>(ddsd.dwHeight), hdcImage, x, y, dx, dy, SRCCOPY);
		pdds->ReleaseDC(hdc);
	}
	DeleteDC(hdcImage);
	return hr;
...
IDirectDrawPalette* DDLoadPalette(IDirectDraw7* pdd, LPCSTR szBitmap)
{
	...
	//
	if (szBitmap && ((h = FindResourceA(NULL, szBitmap, RT_BITMAP)) != 0)) {
		LPBITMAPINFOHEADER lpbi =
			(LPBITMAPINFOHEADER)LockResource(LoadResource(NULL, h));
		if (!lpbi) {
		...
		if (lpbi == NULL || lpbi->biSize < sizeof(BITMAPINFOHEADER)) {
			...
			n = static_cast<int>(lpbi->biClrUsed);
		}
		//
		...
		if (bi.biSize != sizeof(BITMAPINFOHEADER)) {
			...
			n = static_cast<int>(bi.biClrUsed);
		}
		//
		//  A DIB color table has its colors stored BGR not RGB
		//  so flip them around.
