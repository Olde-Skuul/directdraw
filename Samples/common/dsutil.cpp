...
IDirectSoundBuffer* DSLoadSoundBuffer(IDirectSound* pDS, LPCTSTR lpName)
{
	...
	if (DSGetWaveResource(NULL, lpName, &dsBD.lpwfxFormat, &pbWaveData,
			&dsBD.dwBufferBytes)) {
...
IDirectSoundBuffer* SndObjGetFreeBuffer(SNDOBJ* pSO)
{
	...
	IDirectSoundBuffer* pDSB = pSO->Buffers[pSO->iCurrent];

	if (pDSB) {
		DWORD dwStatus;

		HRESULT hres = pDSB->GetStatus(&dwStatus);
...
BOOL DSFillSoundBuffer(
	IDirectSoundBuffer* pDSB, BYTE* pbWaveData, DWORD cbWaveSize)
{
	if (pDSB && pbWaveData && cbWaveSize) {
		LPVOID pMem1, pMem2;
		DWORD dwSize1, dwSize2;

		if (SUCCEEDED(pDSB->Lock(
				0, cbWaveSize, &pMem1, &dwSize1, &pMem2, &dwSize2, 0))) {
