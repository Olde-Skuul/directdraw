...
//-----------------------------------------------------------------------------
static HBITMAP CreateDibBMP(HDC hdc, int w, int h, unsigned short bpp)
{
	LPVOID lpBits;
	struct {
		BITMAPINFOHEADER bi;
	...
	return CreateDIBSection(
		hdc, (LPBITMAPINFO)&dib, DIB_RGB_COLORS, &lpBits, NULL, 0);
}
...
//       Note that this is an internal (not exported) function.
//-----------------------------------------------------------------------------
static HBITMAP CreateBMPFromWindow(HWND hwnd)
{
	...
	HBITMAP hbmBitmap = CreateDibBMP(hdcScreen, cx, cy, 16);
	...
	IDirectDrawSurface7* FrontBuffer, IDirectDrawSurface7* BackBuffer)
{
	...
	structSurfaceSize.cx = static_cast<LONG>(ddsd.dwWidth);
	structSurfaceSize.cy = static_cast<LONG>(ddsd.dwHeight);
}
...
//-----------------------------------------------------------------------------
HWND FSWindow_Begin(HWND hwnd, BOOL bStaticContent)
{
	RECT rc;
...
void FSWindow_Update()
{
	...
	if (bNonGDI) {
		...
		// Get a DC to the backbuffer on the non-GDI device (where we need to
		// copy it)
		HDC hdcScreen = GetDC(NULL);
		ddBackBuffer->GetDC(&hdcBackBuffer);

		// If window has a complex region associated with it, be sure to include
		// it in the draw
...
// Special case for potentially quirky non-GDI drivers
#if 0
			// If content is dynamic (updated each frame), always grab the
			// screen copy by calling CreateBMPFromWindow to update image held
			// in Bitmap
			HDC hdcMemory = CreateCompatibleDC(NULL);
		...
		HCURSOR MouseCursorCur = GetCursor();
