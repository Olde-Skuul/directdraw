...
{
	SetClassLongPtrA(GetSafeHwnd(), GCLP_HBRBACKGROUND,
		(LONG_PTR)CreateSolidBrush(RGB(100, 100, 120)));
...
HRESULT CDxtexView::BuildViewSurface(BOOL bOrig, DWORD dwCubeMapFace, LONG lwMip, BOOL bViewAlpha)
{
	...
	// Get correct mip level
	{
		...
		for (i = 0; i < lwMip; i++)
		{
			if (FAILED(hr2 = pdds->GetAttachedSurface(&ddsCaps, &pdds2)))
				return hr2;
			ReleasePpo(&pdds);
			pdds = pdds2;
		}
