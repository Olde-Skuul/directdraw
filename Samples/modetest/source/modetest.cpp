...
HRESULT OnModeTest();
HRESULT PerformDirectDrawModeTest(
...
//-----------------------------------------------------------------------------
BOOL WINAPI DDEnumCallbackEx(GUID* pGUID, LPSTR strDriverDescription,
...
VOID OnInitDialog()
{
	...
	PostMessageA(g_hDlg, WM_SETICON, ICON_BIG,
		reinterpret_cast<LPARAM>(hIcon)); // Set big icon
	PostMessageA(g_hDlg, WM_SETICON, ICON_SMALL,
		reinterpret_cast<LPARAM>(hIcon)); // Set small icon
...
HRESULT UpdateModesListBox(DWORD dwDeviceIndex)
{
	...
	for (DWORD i = 0; i < g_aDevices[dwDeviceIndex].dwModeCount; i++) {
		...
		SendMessage(hWndModesToTest, LB_ADDSTRING, 0,
			reinterpret_cast<LPARAM>(strMode));
		SendMessage(hWndModesToTest, LB_SETITEMDATA, i,
			static_cast<LPARAM>(static_cast<int>(i)));
	}
...
HRESULT OnModeTest()
{
	...
	if (dwSelectCount > 0) {
		...
		default:
			if (SUCCEEDED(hr)) {
				MessageBox(
					g_hDlg, TEXT("Mode test completed"), TEXT("Result"), MB_OK);
				break;
			} else {
		...
		if (FAILED(hr = UpdateModesListBox(static_cast<DWORD>(dwDeviceIndex))))
			return hr;
	} else {
